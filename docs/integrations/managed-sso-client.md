...
- Clients present in Keycloak/Vault but absent from App-Interface are deleted; there is no "orphan protection" opt-out for this integration (unlike some other integrations) since Vault is the sole source of truth for current state - **except** when the App-Interface GraphQL query returns *zero* clients entirely (not "this one client is absent", but the whole result set is empty). The client-side integration treats that case as a failsafe no-op (logs a warning, never calls the server at all) rather than a delete-all, to avoid a transient/misbehaving upstream response being misread as "delete every managed SSO client". This is the same pattern used by several other client-orchestrated integrations in this repo (e.g. `github_owners_api`, `quay_repos_api`)
- **Known limitation:** because of the failsafe above, removing the *last* remaining managed-sso-client declaration in all of App-Interface does not get cleaned up automatically - the reconciler is never invoked with an empty desired set. The client and its Vault secrets become orphaned and require manual cleanup: delete the client from Keycloak (admin console or `DELETE /clients-registrations/default/{clientId}` with its `registration_access_token`), then delete both Vault secrets - the AppSRE-owned one at `<management prefix>/<client_id>` and the tenant-facing one at its `output` path (or `<default output prefix>/<client_id>` if unset). This limitation only applies to the very last client; removing any client while at least one other still exists reconciles normally
