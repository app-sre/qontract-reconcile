"""Unit tests for OCM groups reconciliation router endpoints."""
...
ENDPOINT = "/api/v1/integrations/ocm-groups/reconcile"
...
@pytest.fixture
def sample_reconcile_request() -> OcmGroupsReconcileRequest:
    return OcmGroupsReconcileRequest(
        ...
        desired_state=[
            {"cluster": "cluster-1", "group": "dedicated-admins", "user": "alice"}
        ],
        dry_run=True,
    )


@patch("qontract_api.integrations.ocm_groups.router.reconcile_ocm_groups_task")
def test_post_reconcile_queues_task(
    mock_task: MagicMock,
    client: TestClient,
    auth_headers: dict[str, str],
    sample_reconcile_request: OcmGroupsReconcileRequest,
) -> None:
...
def test_post_reconcile_requires_auth(
    client: TestClient, sample_reconcile_request: OcmGroupsReconcileRequest
) -> None:
...
@patch("qontract_api.integrations.ocm_groups.router.wait_for_task_completion")
def test_get_task_status(
    mock_wait: MagicMock, client: TestClient, auth_headers: dict[str, str]
) -> None:
    expected_result = OcmGroupsTaskResult(
