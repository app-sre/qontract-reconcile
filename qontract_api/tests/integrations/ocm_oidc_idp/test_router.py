"""Unit tests for OCM OIDC identity provider reconciliation router endpoints."""
...
ENDPOINT = "/api/v1/integrations/ocm-oidc-idp/reconcile"
...
@pytest.fixture
def sample_reconcile_request() -> OcmOidcIdpReconcileRequest:
    return OcmOidcIdpReconcileRequest(
...
@patch("qontract_api.integrations.ocm_oidc_idp.router.reconcile_ocm_oidc_idp_task")
def test_post_reconcile_queues_task(
    mock_task: MagicMock,
    client: TestClient,
    auth_headers: dict[str, str],
    sample_reconcile_request: OcmOidcIdpReconcileRequest,
) -> None:
...
def test_post_reconcile_requires_auth(
    client: TestClient, sample_reconcile_request: OcmOidcIdpReconcileRequest
) -> None:
...
@patch("qontract_api.integrations.ocm_oidc_idp.router.wait_for_task_completion")
def test_get_task_status(
    mock_wait: MagicMock, client: TestClient, auth_headers: dict[str, str]
) -> None:
    expected_result = OcmOidcIdpTaskResult(
