...
Apply rate limiting in API client (see ADR-004):

```python
class PagerDutyClient:
    """PagerDuty API client with rate limiting."""

    def __init__(self, token: str, hooks: list[Callable] | None = None):
        self.token = token
        self.hooks = hooks or []

    def get_schedule_users(self, schedule_id: str) -> list[str]:
        """Get users from schedule (rate-limited via hooks)."""
        # Call hooks (rate limiting applied here)
        for hook in self.hooks:
            hook("GET", f"/schedules/{schedule_id}/users")
...
```python
@router.get("/schedules/{schedule_id}/users")
def get_schedule_users(schedule_id: str, cache: CacheBackend = Depends(get_cache)):
    """Get schedule users with monitoring."""
    cache_key = f"pagerduty:schedule:{schedule_id}:users"

    # Log cache access
    if cached := cache.get(cache_key):
        logger.info(
            "PagerDuty schedule cache hit",
            extra={
    ...
    # Log cache miss and API call
    logger.info(
        "PagerDuty schedule cache miss - calling API",
        extra={
            "schedule_id": schedule_id,
            "cache_hit": False,
        }
...
### Pattern 6: Error Handling in Integrations

Handle qontract-api errors gracefully:
...
def get_pagerduty_schedule_users(schedule_id: str) -> list[str]:
    ...
    try:
        ...
        logger.error(f"Timeout calling qontract-api for schedule {schedule_id}")
        # Graceful degradation: return empty list or cached data
        return []

    except requests.exceptions.HTTPError as e:
        logger.error(f"HTTP error from qontract-api: {e}")
        # Propagate or return fallback
        raise
```

### Checklist

- [ ] Create qontract-api endpoint for external API
- [ ] Implement caching with appropriate TTL
- [ ] Add rate limiting via client hooks
- [ ] Add monitoring/logging for cache hits/misses
- [ ] Update integration to call qontract-api endpoint
- [ ] Remove direct external API calls from integration
...
**After (Via qontract-api - PLANNED):**
...
**When to Create qontract-api Endpoints:**

Create endpoint when:

- Data is used by multiple integrations
- External API has rate limits
- Data changes infrequently (can be cached)
- Consistent access pattern across consumers

Don't create endpoint for:

- One-off data access (single integration)
- Real-time data requirements (no caching benefit)
- Simple data transformations (better in integration)
