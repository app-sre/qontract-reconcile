...
- [ ] Configure `QAPI_LOG_FORMAT_JSON` environment variable
- [ ] Add `RequestIDMiddleware` to FastAPI app
- [ ] Use `extra` parameter for structured fields
- [ ] Test both JSON and standard modes
- [ ] Verify request ID appears in all logs
- [ ] Configure log aggregation system to parse JSON
...
Context variables (`contextvars.ContextVar`) are thread-safe and async-safe, making them ideal for request ID tracking in FastAPI (which uses thread pools for sync endpoints and async for async endpoints).
...
No migration needed - this is a POC. For existing systems:

1. Deploy with `QAPI_LOG_FORMAT_JSON=false` first (standard mode)
2. Verify all logs work correctly
3. Switch to `QAPI_LOG_FORMAT_JSON=true` (JSON mode)
4. Update log aggregation dashboards to use JSON fields
