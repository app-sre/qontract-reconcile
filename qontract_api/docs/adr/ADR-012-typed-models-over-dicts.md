...
- [ ] Use `BaseModel` for all API request/response structures
- [ ] Add `model_config = ConfigDict(frozen=True)` to all request/response models
- [ ] Each nesting level has dedicated model class
- [ ] No `dict[str, dict[str, X]]` for structured data
- [ ] Use `Field(default_factory=list)` for list defaults
- [ ] Avoid data redundancy across model levels
- [ ] Add docstrings to model classes
- [ ] Test with `.model_validate()` and `.model_dump()`
- [ ] Use `.model_copy(update={...})` to modify frozen models
...
**Example: Slack Usergroups Models**

The slack_usergroups integration uses a three-level hierarchy with frozen models:

```python
from pydantic import BaseModel, ConfigDict, Field

# Level 3: Configuration (leaf)
class SlackUsergroupConfig(BaseModel):
    model_config = ConfigDict(frozen=True)
...
- Full type safety from root to leaf
- IDE autocompletion at every level
- Automatic validation of entire structure
