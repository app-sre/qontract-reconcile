...
- Fetching desired_state from qontract-server (GraphQL)
...
- Related: [ADR-003](ADR-003-direct-vs-queued-execution-modes.md) - Execution modes (both use client-provided desired_state)
- Implementation: Client in `reconcile/slack_usergroups_api.py`, API in `qontract_api/integrations/slack_usergroups/`
