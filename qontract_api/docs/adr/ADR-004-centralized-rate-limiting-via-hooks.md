...
┌─────────────────────────────────────────────────────────────┐
│ Direct Integration (reconcile/slack_usergroups.py)          │
│  └─> slack_api = SlackApi(workspace, token)                 │
│       └─> NO rate limiting (backward compatible) ✓          │
└─────────────────────────────────────────────────────────────┘
...
- Related: [ADR-005](ADR-005-sync-first-async-when-needed.md) - Sync-First Development (TokenBucket uses sync methods)
