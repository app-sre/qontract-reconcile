...
class GitHubApiCallContext:
    method: str        # e.g., "repos.get_pull"
    verb: str          # e.g., "GET"
    repository: str    # e.g., "owner/repo"
    organization: str

def _github_metrics_hook(context: GitHubApiCallContext) -> None:
    github_request.labels(context.method, context.verb, context.organization).inc()
```
...
def _gitlab_metrics_hook(context: GitLabApiCallContext) -> None:
    gitlab_request.labels(context.method, context.verb, context.group).inc()
```
...
class AWSApiCallContext:
    method: str       # e.g., "describe_instances"
    service: str      # e.g., "ec2", "s3", "iam"
    region: str
    account_id: str

def _aws_metrics_hook(context: AWSApiCallContext) -> None:
    aws_request.labels(context.service, context.method, context.region).inc()
```
...
- Related: [ADR-005](ADR-005-sync-first-async-when-needed.md) - Sync-First Development (hooks are sync-only)
