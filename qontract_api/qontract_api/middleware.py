...
from qontract_api.logger import logger, request_id_context
    ...
    async def dispatch(  # noqa: PLR6301 - Required instance method for Starlette middleware
        ...
        if request.headers.get("content-encoding") == "gzip":
            try:
                ...
                logger.error(
                    f"Failed to decompress gzip request: {e}",
                    extra={"error": str(e)},
                )
                return Response(
            ...
            except Exception as e:  # noqa: BLE001 - Catch-all for safety, log details
                logger.error(
                    f"Unexpected error decompressing request: {e}",
                    extra={"error": str(e)},
                )
                return Response(
