...
def construct_gabi_oc_resource(name: str,
                               users: Iterable[str]) -> OpenshiftResource:
    body = {
        "apiVersion": "v1",
        "kind": "ConfigMap",
        "metadata": {
    ...
    return OpenshiftResource(body, QONTRACT_INTEGRATION,
                             QONTRACT_INTEGRATION_VERSION, error_details=name)
...
def run(dry_run: bool, thread_pool_size: int = 10,
    ...
    if not gabi_instances:
        logging.debug(
            "No gabi instances found in app-interface")
        sys.exit(ExitCodes.SUCCESS)

    gabi_namespaces = [
        i['namespace'] for g in gabi_instances for i in g['instances']
    ]
