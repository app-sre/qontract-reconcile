...
def test_unpack_static_variables_json_backward_compatibility(
    gql_class_factory: Callable,
) -> None:
    """Test that non-template variables use JSON processing"""
    collection_variables = gql_class_factory(
        TemplateCollectionVariablesV1,
        {
            "static": '{"string_value": "simple string", "number_value": 42, "boolean_value": true, "list_value": ["item1", "item2"], "dict_value": {"key": "value", "nested": {"count": 3}}}',
            "dynamic": [],
        },
    )
    result = unpack_static_variables(collection_variables, {})
    assert result == {
        "string_value": "simple string",
        "number_value": 42,
        "boolean_value": True,
        "list_value": ["item1", "item2"],
        "dict_value": {"key": "value", "nested": {"count": 3}},
    }


def test_unpack_static_variables_multiline_prometheus_query(
    gql_class_factory: Callable,
) -> None:
    """Test that multi-line Prometheus queries with Jinja2 templates are processed correctly.

    This addresses the issue where multi-line template strings were failing because
    the original JSON round-trip approach would escape newlines and break the template processing.
    """
    collection_variables = gql_class_factory(
        TemplateCollectionVariablesV1,
        {
            "static": '{"slo_error_query": "{{ each.slo_error_query }}", "slo_total_query": "{{ each.slo_total_query }}", "slo_name": "{{ each.slo_name }}"}',
            "dynamic": [],
        },
    )
    each = {
        "slo_error_query": """sum(rate(qontract_reconcile_function_elapsed_seconds_since_bundle_commit_count{
          integration!~"terraform-resources|sql-query",
          namespace="prod"
        }[{{window}}]))
        -
        sum(rate(qontract_reconcile_function_elapsed_seconds_since_bundle_commit_bucket{
          le="1200.0",
          integration!~"terraform-resources|sql-query",
          namespace="prod"
        }[{{window}}]))""",
        "slo_total_query": """sum(rate(qontract_reconcile_function_elapsed_seconds_since_bundle_commit_count{
          integration!~"terraform-resources|sql-query",
          namespace="prod"
        }[{{window}}]))""",
        "slo_name": "ReconciliationTimeAppInterfaceIntegrations",
    }

    result = unpack_static_variables(collection_variables, each)
    # Verify that the multi-line queries are processed correctly and preserve formatting
    assert "slo_error_query" in result
    assert "slo_total_query" in result
    assert "slo_name" in result
    # Check that newlines and complex formatting are preserved
    assert "\n" in result["slo_error_query"]
    assert (
        "qontract_reconcile_function_elapsed_seconds_since_bundle_commit_count"
        in result["slo_error_query"]
    )
    assert (
        "{{window}}" in result["slo_error_query"]
    )  # Template placeholders should remain
    assert "\n" in result["slo_total_query"]
    assert (
        "qontract_reconcile_function_elapsed_seconds_since_bundle_commit_count"
        in result["slo_total_query"]
    )
    assert result["slo_name"] == "ReconciliationTimeAppInterfaceIntegrations"
