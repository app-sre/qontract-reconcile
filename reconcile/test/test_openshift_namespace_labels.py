...
def run_integration(dry_run=False, thread_pool_size=1,
                    internal=None, use_jump_host=True,
                    raise_errors=True):
    """Calls the integration with sensible overridable defaults"""
    openshift_namespace_labels.run(dry_run=dry_run,
...
class TestOpenshiftNamespaceLabels(TestCase):
    ...
    def _oc_map_get(self, cluster):
        ...
        ns = [ns.oc_get_all() for ns in self.test_ns
    ...
    def _state_ls(self):
        ...
        return [f'/{ns.state_key()}' for ns in self.test_ns
                if ns.managed is not None]
    ...
    def test_multi_cluster(self):
        ...
        calls = [call(state_key(c1, 'multi-cluster'), k1_k2, force=True),
