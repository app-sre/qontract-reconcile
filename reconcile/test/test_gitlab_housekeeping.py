...
def test_bot_note_does_not_clear_merge_error(
...
def test_merge_error_cleared_when_rebased_with_passing_pipeline(
    mocker: MockerFixture, project: Project
) -> None:
    """merge-error is removed when the MR is rebased and the latest
    pipeline has succeeded, even without any human comment."""
    mr = _make_healthcheck_mr(labels=["lgtm", "merge-error"])
    mocked_gl = create_autospec(GitLabApi)
    mocked_gl.project = project
    ...
    mocked_gl.remove_label.assert_called_once_with(mr, "merge-error")
    mocked_gl.get_merge_request_label_events.assert_not_called()


def test_merge_error_stays_when_rebased_but_pipeline_failed(
    mocker: MockerFixture, project: Project
) -> None:
    """merge-error stays when the MR is rebased but the latest pipeline
    has not succeeded."""
    mr = _make_healthcheck_mr(labels=["lgtm", "merge-error"])
    label_event = create_autospec(ProjectMergeRequestResourceLabelEvent)
    label_event.action = "add"
...
def test_merge_error_stays_when_not_rebased_with_passing_pipeline(
    mocker: MockerFixture, project: Project
) -> None:
    """merge-error stays when the pipeline passes but the MR is not rebased
    and no human has commented."""
    mr = _make_healthcheck_mr(labels=["lgtm", "merge-error"])
    label_event = create_autospec(ProjectMergeRequestResourceLabelEvent)
    label_event.action = "add"
    label_event.label = {"name": "merge-error"}
