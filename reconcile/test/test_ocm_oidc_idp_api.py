...
class TestBuildClusters:
    ...
    def test_enforced_status(self) -> None:
        result = build_clusters(
            [make_ocm_cluster(labels={"sre-capabilities.rhidp.status": "enforced"})],
            "redhat-sso",
            "https://default-issuer.example.com",
        )

        assert result[0].auth.oidc_enabled is True
        assert result[0].auth.enforced is True

    def test_sso_client_only_status_disables_oidc(self) -> None:
        result = build_clusters(
            [
                make_ocm_cluster(
                    labels={"sre-capabilities.rhidp.status": "sso-client-only"}
                )
            ],
            "redhat-sso",
            "https://default-issuer.example.com",
        ...
        assert result[0].auth.oidc_enabled is False
        assert result[0].auth.enforced is False
    ...
    def test_disabled_clusters_still_included(self) -> None:
        """Disabled clusters must still be sent, so the server can still diff and
        clean up any leftover identity provider from before the cluster was disabled.
        """
        result = build_clusters(
...
class TestAsyncRun:
    def _make_task_response(self, task_id: str = "task-123") -> OcmOidcIdpTaskResponse:
    ...
    async def test_dry_run_waits_for_task_and_logs_actions(self) -> None:
        ...
        action = OcmOidcIdpActionCreate(
            cluster_name="my-cluster", auth_name="redhat-sso"
        )
