...
class QuayReposIntegration(
    QontractReconcileApiIntegration[QuayReposIntegrationParams]
):
    ...
    @staticmethod
    def get_quay_orgs(query_func: object) -> list[QuayOrgV1]:
        data = quay_orgs_query(query_func=query_func)
        return list(data.quay_orgs or [])

    @staticmethod
    def get_apps(query_func: object) -> list[AppV1]:
        data = apps_query(query_func=query_func)
        return list(data.apps or [])

    @staticmethod
    def _build_repos_map(apps: list[AppV1]) -> dict[tuple[str, str], list[QuayRepoItemV1]]:
        """Build a map of (instance_name, org_name) -> list of repo items from apps."""
        repos_map: dict[tuple[str, str], list[QuayRepoItemV1]] = {}
        for app in apps:
            for quay_repo in app.quay_repos or []:
                key = (quay_repo.org.instance.name, quay_repo.org.name)
    ...
    def compile_desired_state(
        ...
        for org in orgs:
            ...
            if self.params.instance_name and org.instance.name != self.params.instance_name:
                continue
