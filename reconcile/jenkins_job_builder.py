...
def collect_saas_file_configs(settings, instance_name=None):
    ...
    for saas_file in saas_files:
        ...
        slack_channel = (
            saas_file["slack"]["channel"] if output == "publish" else "dev-null"
        )
            ...
            for target in resource_template["targets"]:
                ...
                jc_data = {
                    "project": {
                        ...
                        "slack_notify_start": slack_notify_start,
                        "upstream": upstream,
                        "jobs": [{final_job_template_name: {"display_name": jc_name}}],
                    }
                }
                if timeout:
...
def collect_configs(instance_name, config_name, settings):
    ...
    if instance_name is not None:
        raw_jjb_configs = [
            n for n in raw_jjb_configs if n["instance"]["name"] == instance_name
        ]
    if config_name is not None:
        raw_jjb_configs = [
            n
            for n in raw_jjb_configs
    ...
    saas_file_configs, saas_file_repo_urls = collect_saas_file_configs(
...
def init_jjb(instance_name=None, config_name=None, print_only=False):
    ...
    configs, additional_repo_urls = collect_configs(
        instance_name, config_name, settings
    )
    return (
        JJB(configs, ssl_verify=False, settings=settings, print_only=print_only),
...
def run(
    ...
    jjb, additional_repo_urls = init_jjb(instance_name, config_name, print_only)
    defer(jjb.cleanup)
