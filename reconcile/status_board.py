...
class StatusBoardExporterIntegration(QontractReconcileIntegration):
    ...
    def run(self, dry_run: bool) -> None:
        ...
        for sb in get_status_board():
            ...
            abstract_status_board_map: dict[str, dict[str, Any]] = {}
            for product, apps in desired_product_apps.items():
                abstract_status_board_map[product] = {
                    "product": product,
                    "app": "",
                }
                for a in apps:
                    abstract_status_board_map[f"{product}/{a}"] = {
                        "product": product,
                        "app": a,
                    }
            for slodoc in slodocs:
                ...
                for slo in slodoc.slos or []:
                    for product in products:
                        ...
                        # Or it is status board disabled
                        if (
                            product not in desired_product_apps
                            ...
                            or (
                                slodoc.labels
                                and "statusBoard" in slodoc.labels
                                and slodoc.labels["statusBoard"]
                            )
                        ):
                            continue

                        abstract_status_board_map[
                            f"{product}/{slodoc.app.name}/{slo.name}"
                        ] = {
                            "product": product,
                            "app": app,
                            "slo": slo.name,
