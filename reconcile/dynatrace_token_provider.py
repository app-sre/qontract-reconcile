...
class DynatraceTokenProviderIntegration(
            ...
            for env in self.get_ocm_environments():
                ...
                with DTPOrganizationErrorRate(
                    ...
                    for cluster in clusters:
                        if cluster.organization_id not in self.params.ocm_organization_ids:
                            continue
                        ...
                        if cluster.labels:
                            ...
                            try:
                                ...
                                unhandled_exceptions.append(f"{env}/{cluster.organization_id}: {e}")
                            ...
                            if existing_syncsets["size"] == 0 or not [
                                ...
                                if not dry_run:
                                    ...
                                    logging.info(f"Tokens: {ingestion_token.id},{operator_token.id} created in Dynatrace environment: {dt_tenant_id}")
                                else:
                                    logging.info(f"Would have created ingestion and operation token in Dynatrace environment: {dt_tenant_id}")
                                                    ...
                                                    if "does not exist" in e.args[0]:
                                                        if (
                                                            ...
                                                            if not dry_run:
                                                                ...
                                                                logging.info(f"Token: {ingestion_token.id} created in Dynatrace environment: {dt_tenant_id}")
                                                            else:
                                                                logging.info("Would create ingestion token in Dynatrace")
                                                        elif (
                                                            "operator"
                                                            in resource["metadata"]["name"]
                                                        ):
                                                            if not dry_run:
                                                                ...
                                                                logging.info(f"Token: {operator_token.id} created in Dynatrace environment: {dt_tenant_id}")
                                                            else:
                                                                logging.info("Would create operator token in Dynatrace")
                                                    unhandled_exceptions.append(f"{env}/{cluster.organization_id}: {e}")
                                        break
                            ...
                            if not dry_run:
                                try:
                                    ...
                                    _expose_errors_as_service_log(cluster.ocm_cluster.external_id, str(e.args)) 
                                    unhandled_exceptions.append(f"{env}/{cluster.organization_id}: {e}")
                            else:
    ...
    def create_dynatrace_ingestion_token(self, dt_client: Dynatrace, cluster_uuid: str):
        return dt_client.tokens.create(
            name=f"ingestion-token-{cluster_uuid}",
            scopes=["metrics.ingest", "logs.ingest", "events.ingest"],
        )

    def create_dynatrace_operator_token(self, dt_client: Dynatrace, cluster_uuid: str):
        return dt_client.tokens.create(
    ...
    def create_dynatrace_tokens(self, dt_client: Dynatrace, cluster_uuid: str):
