...
from reconcile.fleet_labeler.merge_request import FleetLabelerUpdates, YamlCluster
...
class FleetLabelerIntegration(QontractReconcileIntegration[NoParams]):
    ...
    def _sync_cluster_inventory(
        ...
        current_cluster_ids = [cluster.cluster_id for cluster in spec.clusters]
        clusters_to_add: list[YamlCluster] = []
        cluster_ids_to_delete: list[str] = []
        for label_default in spec.label_defaults:
            ...
            cluster_ids_to_delete.extend(diff.delete.values())
            for cluster_id in diff.add:
                # We want to query cluster labels only for clusters that need to be added
                ...
                clusters_to_add.append(
                    YamlCluster(
                        ...
                        subscription_labels=self._render_default_labels(
        ...
        current_content = vcs.get_file_content_from_app_interface_master(
            file_path=spec.path
        )
        desired_content = self._render_yaml_file(
            current_content, cluster_ids_to_delete, clusters_to_add
        )
        mr = FleetLabelerUpdates(path=spec.path, content=desired_content)
        # Note, that VCS is initialized with dry-run flag already
        vcs.open_app_interface_merge_request(mr)
