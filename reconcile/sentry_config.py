  ...
  def reconcile(self, current, desired):
    ...
    for user in desired.users.keys():
      ...
      if user not in current.users.keys():
        logging.info(["add_user", user, ",".join(teams), self.client.host])
        if not self.dry_run:
          self.client.create_user(user, "member", teams)
      else:
        logging.info(["team_membership", user, ",".join(teams), self.client.host])
        if not self.dry_run:
    ...
    for team in desired.projects.keys():
      for desired_project in desired.projects[team]:
        ...
        logging.info(["project_config", desired_project, self.client.host])
        if not self.dry_run:
...
def fetch_desired_state(gqlapi, sentry_instance):
  ...
  for role in result['roles']:
    ...
    for team in role['sentry_teams']:
      ...
      # Only add users if the team they are a part of is in the same senry
      # instance we are querying for information
...
def run(dry_run=False):
  ...
  for instance in result['instances']:
    token = secret_reader.read(instance['automationToken'], settings=settings)
    host = instance['consoleUrl']
    sentry_client = SentryClient(host, token)

    skip_user = secret_reader.read(instance['adminUser'], settings=settings)
