...
def run_test(test):
    test['check_result'] = promtool.run_test(test_yaml_spec=test['test'],
                                             rule_files=test['rule_files'])
    return test
...
def check_prometheus_tests(tests, rules, thread_pool_size):
    ...
    for path, test in tests.items():
        ...
        if not rule_files:
            failed_tests.append({**test_to_run,
                                 'check_result': 'Cannot parse test'})
            continue

        # this makes things so much simpler. Let's revisit it if we need
        if len(rule_files) > 1:
            failed_tests.append({**test_to_run,
                                 'check_result': 'Only 1 rule file per test'})
            continue
        ...
        try:
            ...
            msg = f'Error in test template for cluster {cluster} and ' \
                  f'namespace {namespace}: {e}'
            failed_tests.append({**test_to_run, 'check_result': msg})
            continue

    result = threaded.run(func=run_test,
                          iterable=tests_to_run,
                          thread_pool_size=thread_pool_size)
...
def run(dry_run, thread_pool_size=10, cluster_name=None):
    ...
    if failed_tests:
        for f in failed_tests:
            ...
            if 'cluster' in f:
                msg += f" from namespace {f['namespace']} in cluster" \
                       f"{f['cluster']}"
