...
from typing import Any, Dict, List, Iterable, Mapping, MutableMapping, Optional
...
class TerrascriptClient:
    def __init__(
        self, integration, integration_prefix, thread_pool_size, accounts, settings=None
    ):
    ...
    def populate_configs(self, accounts):
        results = threaded.run(self.get_tf_secrets, accounts, self.thread_pool_size)
    ...
    def get_tf_secrets(self, account):
        ...
        secret["supportedDeploymentRegions"] = account["supportedDeploymentRegions"]
        secret["resourcesDefaultRegion"] = account["resourcesDefaultRegion"]
    ...
    def populate_tgw_attachments(self, desired_state):
        for item in desired_state:
            ...
            acc_account_name = acc_account["name"]
            acc_alias = self.get_alias_name_from_assume_role(acc_account["assume_role"])
            acc_uid = self.get_alias_uid_from_assume_role(acc_account["assume_role"])
    ...
    def populate_resources(
        self, namespaces, existing_secrets, account_name, ocm_map=None
    ):
    ...
    def populate_tf_resources(self, populate_spec, existing_secrets, ocm_map=None):
        ...
        if provision_provider != "aws":
            return
        if provider == "rds":
            self.populate_tf_resource_rds(resource, namespace_info, existing_secrets)
        elif provider == "s3":
            self.populate_tf_resource_s3(resource, namespace_info)
        elif provider == "elasticache":
            self.populate_tf_resource_elasticache(
                resource, namespace_info, existing_secrets
            )
        elif provider == "aws-iam-service-account":
            self.populate_tf_resource_service_account(
                resource, namespace_info, ocm_map=ocm_map
            )
            ...
            self.populate_tf_resource_s3_cloudfront_public_key(resource, namespace_info)
        elif provider == "alb":
            self.populate_tf_resource_alb(resource, namespace_info, ocm_map=ocm_map)
    ...
    def populate_tf_resource_rds(self, resource, namespace_info, existing_secrets):
        (
            account,
            identifier,
            values,
        ...
        if self._db_needs_auth(values):
            reset_password = self._should_reset_password(
                reset_password_current_value, existing_secrets, account, output_prefix
            )
            if reset_password:
        ...
        if replica_source:
            ...
            if source_info:
                ...
                deps.append("aws_db_instance." + source_info["resource"]["identifier"])
        ...
        if kms_key_id is not None:
            if kms_key_id.startswith("arn:"):
                ...
                if kms_key:
                    kms_res = "aws_kms_key." + kms_key["resource"]["identifier"]
    ...
    @staticmethod
    def _should_reset_password(current_value, existing_secrets, account, output_prefix):
        """
        ...
        if current_value:
            try:
                existing_secret = existing_secrets[account][output_prefix]
                existing_value = existing_secret["reset_password"]
            except KeyError:
                existing_value = None
    ...
    def populate_tf_resource_elasticache(
        self, resource, namespace_info, existing_secrets
    ):
        (
            account,
            identifier,
    ...
    def populate_tf_resource_service_account(
        self, resource, namespace_info, ocm_map=None
    ):
        (
            account,
            identifier,
            ...
            if assume_role:
                ...
                for user_arn, _, state, switch_role_link in role_grants:
                    ...
                    if identifier == user_id and state != "failed":
                        switch_role_arn = self.get_role_arn_from_role_link(
                            switch_role_link
                        )
                        output_name_0_13 = output_prefix + "__role_arn"
                        tf_resources.append(
            ...
            for queue_kv in queues:
                ...
                if kms_master_key_id is not None:
                    if kms_master_key_id.startswith("arn:"):
                        ...
                        kms_key = self._find_resource(account, kms_master_key_id, "kms")
                        if kms_key:
                            kms_res = "aws_kms_key." + kms_key["resource"]["identifier"]
    ...
    def populate_tf_resource_s3_cloudfront(self, resource, namespace_info):
        ...
        bucket_tf_resource = self.populate_tf_resource_s3(resource, namespace_info)
    ...
    def populate_tf_resource_s3_cloudfront_public_key(self, resource, namespace_info):
        (
            account,
            identifier,
            common_values,
    ...
    def populate_tf_resource_alb(self, resource, namespace_info, ocm_map=None):
        (
            account,
            identifier,
            common_values,
