...
class TerraformClient:  # pylint: disable=too-many-public-methods
    ...
    def populate_desired_state(self, ri, oc_map, tf_namespaces, account_name):
        ...
        for account, output in self.outputs.items():
            ...
            formatted_output = self.format_output(output, self.OUTPUT_TYPE_SECRETS)
            ...
            for name, data in formatted_output.items():
                ...
                if account in replicas_info:
                    if name in replicas_info[account]:
                        ...
                        data["db.user"] = formatted_output[replica_src_name]["db.user"]
                        data["db.password"] = formatted_output[replica_src_name][
                            "db.password"
                        ]
                ...
                if not oc_map.get(cluster):
                    continue
                namespace = data["{}_namespace".format(self.integration_prefix)]
                resource = data["{}_resource".format(self.integration_prefix)]
                output_resource_name = data[
                    "{}_output_resource_name".format(self.integration_prefix)
                ]
                annotations = data.get("{}_annotations".format(self.integration_prefix))

                oc_resource = self.construct_oc_resource(
                    output_resource_name, data, account, annotations
                )
                ri.add_desired(
                    cluster, namespace, resource, output_resource_name, oc_resource
                )
            ...
            for tf_resource in tf_namespace["terraformResources"]:
                ...
                if replica_src is None:
                    ...
                    if defaults_ref is not None:
                        defaults_res = gql.get_resource(defaults_ref)
                ...
                # self.format_output()
                replica_name = (
                    f'{tf_resource.get("identifier")}-' f'{tf_resource.get("provider")}'
                )

                # The replica source name, as found in the
                # self.format_output()
                replica_source_name = f"{replica_src}-" f'{tf_resource.get("provider")}'
