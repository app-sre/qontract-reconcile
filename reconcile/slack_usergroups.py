...
from reconcile.gql_definitions.slack_usergroups.pagerduty_instances import (
    PagerDutyInstanceV1,
)
from reconcile.gql_definitions.slack_usergroups.pagerduty_instances import (
...
from reconcile.gql_definitions.slack_usergroups.users import User
...
def get_pagerduty_map(
    pagerduty_instances: Iterable[PagerDutyInstanceV1], init_users: bool = True
) -> PagerDutyMap:
    # settings (for SecretReader) dependency will be addressed later (https://issues.redhat.com/browse/APPSRE-6596)
    settings = queries.get_app_interface_settings()
    return PagerDutyMap(
        instances=[
            {"name": i.name, "token": i.token.dict()} for i in pagerduty_instances
        ],
...
def run(
    ...
    pagerduty_map = get_pagerduty_map(
        pagerduty_instances=pagerduty_instances, init_users=init_users
    )
