...
#     def _populate_secret_data(self, specs: Iterable[ExternalResourceSpec]) -> None:
#         for spec in specs:
#             vault_path = f"{self.engine}/{spec.provision_provider}/{spec.provisioner_name}/{spec.provider}/{spec.identifier}"
#             s = VaultSecret(path=vault_path, field="not-used")
#             spec.secret = self.secrets_reader.read_all_secret(s)
#             self._populate_annotations(spec)
