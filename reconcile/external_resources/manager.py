...
class ExternalResourcesManager:
        ...
        if reconciliation.action == Action.APPLY:
            match state.resource_status:
                ...
                case ResourceStatus.CREATED:
                    if reconciliation.resource_digest != state.resource_digest:
                        reconcile = True
                        reason = "Resource spec has changed"
                    elif (
                        ...
                        * 60
                        ...
                        reconcile = True
                        reason = "Resource drift detection run"
        elif reconciliation.action == Action.DESTROY:
            match state.resource_status:
                case ResourceStatus.CREATED:
                    reconcile = True
                    reason = "Resource needs to be deleted"
                case ResourceStatus.ERROR:
                    reconcile = True
                    reason = "Resource status ERROR"
        if reconcile:
    ...
    def _get_desired_objects_reconciliations(self) -> set[Reconciliation]:
        ...
        for key, spec in self.er_inventory.items():
            ...
            try:
                resource = self.build_external_resource(spec, self.er_inventory)
    ...
    def _need_secret_sync(
        ...
        if r.action == Action.APPLY and state.resource_status == ResourceStatus.CREATED:
            return True
        return False
    ...
    def handle_resources(self) -> None:
        ...
        for r in desired_r.union(deleted_r):
            ...
            if self.resource_needs_reconciliation(reconciliation=r, state=state):
                self.reconciler.reconcile_resource(reconciliation=r)
                self._update_state(r, state)

            if to_sync:
                self.sync_secrets(keys=to_sync)

    def sync_secrets(self, keys: Iterable[ExternalResourceKey]) -> None:
    ...
    def handle_dry_run_resources(self) -> None:
        ...
        for r in reconciliations:
            ...
            if state.resource_digest != r.resource_digest:
    ...
    def serialize_resource_input(self, resource: ExternalResource) -> str:
        ...
        return json.dumps(resource.dict())
